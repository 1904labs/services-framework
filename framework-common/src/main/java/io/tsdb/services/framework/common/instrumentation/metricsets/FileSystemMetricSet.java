 ...
 * {@link #getFSName(FileStore)} with an implementation that works on your
 * platform, for example {@link #getFSNameByToStringPath(FileStore)}
 * </p>
 *
...
public class FileSystemMetricSet implements MetricSet {
     ...
     * {@link FileSystems#getDefault()}, skipping those with no usable space.
     */
     ...
     * @param skipZeroAvail Flag about whether or not to include {@link FileStore}s that
     *                      show no usable space
     */
    ...
    public Map<String, Metric> getMetrics() {
        ...
        for (final FileStore store : fileSystems.getFileStores()) {
            try {
                ...
                // threw IOException when tried to get available space. FS is
                // probably not visible anymore, so exclude.
                continue;
            }
    ...
    /**
     * FileStore names are highly implementation specific. Override this
     * function to provide a naming scheme for your platform. Some basic
     * potential implementations are provided.
     *
    ...
    /**
     * FileStore names are highly implementation specific. This function simply
     * escapes the name returned by name().
     *
     * @param store the {@link FileStore} to get name of
    ...
    /**
     * FileStore names are highly implementation specific. This function
     * attempts to extract a name based on the mount point. The mount point is
     * inferred from toString. This is done in a best-effort way, as there is no
     * way to actually get the mount point.
     * <p>
     * This function is for systems where {@link FileStore#toString()} returns a
     * string like {@code / (/dev/disk0s2)} or
     * {@code /mnt/volume1 (/dev/disk0s3)}
     * </p>
     * <p>
     * In other words, Unix-like OSes. The root filesystem, {@code /}, is named
     * "root", all others are named as their mount point path escaped with
     * underscores.
     * </p>
     *
     * @param store the {@link FileStore} to get name of
